...
exports.register = function(req,res){
  ...
  // bcrypt.hash(req.body.password, 5, function( err, bcryptedPassword) {
   ...
   connection.query('INSERT INTO bodylangusers SET ?',users, function (error, results, fields) {
   if (error) {
...
exports.login = function(req,res){
  ...
  connection.query('SELECT * FROM bodylangusers WHERE userid = ?',[userid], function (error, results, fields) {
  if (error) {
    console.log("error ocurred",error);
    res.send({
      "code":400,
